...
public class ResourcesTestUtils {
	...
	private static IInputStreamResourceReader isrr = (InputStream ir) -> {
		try (BufferedReader buffer = new BufferedReader(new InputStreamReader(ir))) {
			return buffer.lines().collect(Collectors.joining("\n"));
		}
	};

	private static IInputStreamResourceReader isrre = new IInputStreamResourceReader() {
		@Override
		public String read(InputStream ir) throws IOException {
			try (BufferedReader buffer = new BufferedReader(new InputStreamReader(ir))) {
				return buffer.lines().collect(Collectors.joining("\n"));
			}
		}
